...
const (
	...
	annotationStatus   = "golden-image.cattle.io/status"      // building | resolved | failed
	annotationJob      = "golden-image.cattle.io/job-name"     // K8s Job name
	annotationSnapshot = "golden-image.cattle.io/snapshot-id"  // resolved snapshot ID
	annotationSpec     = "golden-image.cattle.io/original-spec" // original golden:xxx value
...
// Why: Single source of truth for the controller path is chart/golden-image-controller/values.yaml.
...
func main() {
	...
	cfg := Config{
		BuilderImage: getEnv("BUILDER_IMAGE", "ghcr.io/mbilan1/golden-image-builder:latest"),
		JobNamespace: getEnv("JOB_NAMESPACE", "golden-image-system"),
	...
	setupLog.Info("starting golden-image-controller",
...
func (r *HetznerConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	case "failed":
		...
		log.Info("golden image build failed, waiting for manual intervention", "config", req.Name)
		return ctrl.Result{}, nil
	case "building":
...
func (r *HetznerConfigReconciler) handleNew(ctx context.Context, hc *unstructured.Unstructured, image string) (ctrl.Result, error) {
	...
	enableCIS := spec == "cis"
	log.Info("new golden image request", "config", configName, "spec", spec, "enableCIS", enableCIS)
